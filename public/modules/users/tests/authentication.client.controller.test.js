...
(function() {
	...
	describe('AuthenticationController', function() {
		...
		it('$scope.signup() should fail to register with duplicate Contactname', function() {
			// Test expected POST request
			$httpBackend.when('POST', '/auth/signup').respond(400, {
				'message': 'Contactname already exists'
			});
			...
			// Test scope value
			expect(scope.error).toBe('Contactname already exists');
		});
	});
}());
